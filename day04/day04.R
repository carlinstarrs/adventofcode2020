...
input <- data.frame("X1" = readLines("C:/Users/Carlin/Documents/GitHub/adventofcode2020/day04/input.txt"))

####PART 1####

###TEST 1.1###

passport_test <- function(dat){
  dat %>% 
    mutate(IS_RETURN = X1 == "", 
    ...
    separate(X1, into = c("key", "value"), ":") %>% 
    mutate(key = factor(key, levels = c("byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid", "cid"))) %>% 
    complete(key, id) %>% 
    arrange(id) %>% 
    group_by(id) %>% 
    mutate(IS_MISSING = any(is.na(value[key != "cid"]))) %>% 
    summarise(VALID = sum(IS_MISSING), .groups = "drop") %>% 
    filter(VALID == 0) %>% 
    nrow()
}

passport_test(input_test)
passport_test(input)
...
####PART 2####
###TEST 2.1###
input_test2 <- data.frame("X1" = readLines("C:/Users/Carlin/Documents/GitHub/adventofcode2020/day04/input_test2.txt"))

passport_test2 <- function(dat){
  out <- dat %>% 
    mutate(IS_RETURN = X1 == "", 
           id = cumsum(IS_RETURN) + 1) %>% 
    filter(!IS_RETURN) %>% 
    ...
    group_by(id) %>% 
    mutate(IS_MISSING = any(is.na(value[key != "cid"])), 
           num_val = as.numeric(gsub("[^0-9.]", "", value))) %>% 
    filter(IS_MISSING == FALSE) %>%
    ungroup() %>%
    mutate(TEST_RESULT = case_when(
      key == "byr" ~ nchar(value) == 4 & (value >= 1920 & value <= 2002),
      ...
      key == "pid" ~ nchar(value) == 9, #don't use \\d{9} you dummy
      key == "cid" & is.na(value) ~ TRUE,
      TRUE ~ NA
      )) %>% 
    group_by(id) %>% 
    summarise(VALID = !any(TEST_RESULT[!is.na(TEST_RESULT)] == FALSE), .groups = "drop") %>% 
    filter(VALID == TRUE) %>% 
    nrow()
}

passport_test2(input_test2)
passport_test2(input)
